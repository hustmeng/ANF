...
parser.add_argument('-l', '--lambda', type=float, dest='lamda', default='6',
...
def init_weights(m):
    if isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
        nn.init.normal_(m.weight.data, 0, 0.0001)  # 设置高斯分布的均值为0，标准差为0.01
...
print(f"Params count: {trainable_params}")
        ...
        if (epoch + 1) % print_freq == 0:
            chck.safe_save_checkpoint(model=layers, optimizer=optimizer, scheduler=None, era=era, model_cfg=model_cfg,
                                      **{'mass2': args.mass2, 'lambda': args.lamda},
                                      path=f"phi4_{args.loss}_{L:02d}x{L:02d}_base.zip")
